...
export const DeltaChart: React.FC<DeltaChartProps> = ({ 
  ...
  useEffect(() => {
    ...
    console.log(`DeltaChart mounted - type: ${type}, cumulative: ${cumulative}`);
    console.log(`Data points: ${data?.length || 0}, Container height: ${containerHeight}px`);
    ...
    if (containerHeight <= 0) {
      console.warn('DeltaChart: Container height is 0 or negative, setting to minimum 100px');
    ...
    try {
      ...
      if (cumulative) {
        ...
        const processedData = data.map(item => {
          ...
          const multiplier = item.color.includes('red') || item.color.toLowerCase().includes('ff') && 
                            !item.color.toLowerCase().includes('00ff') ? -1 : 1;
        ...
        const formattedData = data.map(item => {
          ...
          if (item.color) {
            color = item.color.includes('red') || item.color.toLowerCase().includes('ff') && 
                   !item.color.toLowerCase().includes('00ff') ? negativeColor : positiveColor;
          }
          
          return {
            time: ensureSecondsFormat(item.time) as UTCTimestamp,
            value: Math.max(0.01, Math.abs(item.value)), // Ensure minimum value for visibility
            color: color
          };
        });
      ...
      const handleResize = () => {
        if (containerRef.current && chartRef.current) {
          ...
          const newHeight = containerRef.current.clientHeight || Math.max(100, height);
